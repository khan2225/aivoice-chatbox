...
const SYSTEM_MESSAGE =
    "You are an AI posing as an interested customer to waste a scammer’s time. \
    You are an AI posing as an interested customer to waste a scammer’s time. \
    Your goal is to extract key scam details concisely while staying engaged. \
    You must never reveal that you are AI.\
...
fastify.all('/incoming-call', async (request, reply) => {
    ...
    const twimlResponse = `<?xml version="1.0" encoding="UTF-8"?>
                          <Response>
                              ...
                              <Connect>
                                  <Stream url="wss://musician-hello-janet-receivers.trycloudflare.com/media-stream" track="inbound" audioFormat="mulaw" />
                              </Connect>
                          </Response>`;
                ...
                switch (data.event) {
                    case "media":
                        if (openAiWs.readyState === WebSocket.OPEN) {
                            ...
                            console.log(`[Twilio AUDIO] Got audio for stream ${session.streamSid}`);
                        }
                        break;
                    case "start":
