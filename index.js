import Fastify from "fastify";
  ...
  fastify.get("/media-stream", { websocket: true }, async (connection, req) => {
    ...
    try {
        ...
        personaKey = prefData.result?.personaKey || "genZ";
        selectedPersona = PERSONAS[personaKey] || PERSONAS["genZ"];
        if (!PERSONAS[personaKey]) {
            console.warn(`Persona key "${personaKey}" not found. Falling back to genZ.`);
          }
        ...
        console.log("Resolved persona from API:", personaKey);

        console.log("User ID:", session.userId);

    } catch (err) {
        console.warn("Failed to fetch persona, using default.", err);
    }
    ...
    sessions.set(sessionId, session);
      ...
      const sendSessionUpdate = () => {
          ...
          if (openAiWs.readyState === WebSocket.OPEN) {
            ...
            console.warn("Tried to send session update but WebSocket is not open.");
        }
...
connection.on("close", async () => {
    ...
    console.log(`Client disconnected (${sessionId}) for user ${session?.userId}`);
...
fastify.post("/api/v1/app/pull-pref", async (req, reply) => {
    ...
    // Simulated user-persona mapping
